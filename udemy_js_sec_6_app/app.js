...
var UIController = (function() {
  ...
  var formatNumber = function(num, type) {
    ...
    num = Math.abs(num);
    num = num.toFixed(2); 
    // method of Number prototype, gives number with 2 decimal points
    numSplit = num.split('.')
    int = numSplit[0];
    dec = numSplit[1];
    ...
    if(int.length > 3) {
      ...
      // if input 2310 then output is 2,310
    }
  ...
  return {
    ...
    displayBudget: function(obj) {
      ...
      if (obj.percentage > 0) {
        document.querySelector(DOMstrings.percentageLabel).textContent = obj.percentage.toString() + '%';
      } else {
    ...
    displayMonth: function() {
      var now, monthNum, year, month, months;
      now = new Date();
      months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec'];
      monthNum = now.getMonth(); // returns month number
